---
layout: post
title: Intro to GSP - A Signal Processing Perspective For Graph Structured Information
subtitle: Introduction to Graph Signal Processing
tags: [signal-processing, graph, spectral domain]
---

Add here in the beggining 'preferred knowledge' for reading this blog: fourier, convolution, signal processing

> TL;DR: In this post we will learn the basic concepts of linear graph signal processing and see how they're related to classical signal processing. Relevant material is mostly linked and provided, however some very basic background in DSP and Graph spectral theory might simplify the read. By the end of this post the reader will understand what are graph signals, their relation to time domain signals, representation of graph signal in the vertex and the spectral domain, Graph Fourier transform and efficient spectral filtering.

Data appears in various shapes. On the one side we have natural scenes, such as in images, motion trajectories, video or sound, which are unstructured and typically demonstrate continuities, smoothness and regularity. These high-level constraints impose prior knowledge that can be utilized and formulated for modeling these signals. On the other side we meet structured data sources, such as tables and graphs, with discrete associations, irregular properties and relational structure. The structures may seem more complicated and harder to grasp, however their unique structure is highly compact, very efficient and great for search. Specifically, the connectome or wiring network in graphs typically holds rich information on its entities, and thus is interesting to investigate in many different contexts. 
Formally, relational graph data is represented with $$G=\{V,E,W_E\}$$ where $$V$$, $$E$$  and $$W_E$$ denote the group of vertexes (or nodes), edges (links or connections) and edge's weight respectively. In order to make life a bit easier, the graph that we will refer to here will be undirected $$G$$ with $$N$$ nodes and $$E$$ edges.

The first question to ask would probably be - How do we define a signal over a graph?
...
### The Shift operator in Signal Processing
We will start by explaining the shift and its role in DSP and GSP. From now on we will use $$s$$ for a signal with $$N$$ samples. In Discrete Signal processing (DSP), we can represent every finite signal with the [$$z$$-transform](https://en.wikipedia.org/wiki/Z-transform) as a polynomial expression, s.t. 
(I think you should add here a bit more intuition of moving from time dimension to frequancy dimension)
...
In DSP the shift is the foundation for building many linear filters, such as the [Finite Impulse Response (FIR)](https://en.wikipedia.org/wiki/Finite_impulse_response) filter family, which are defined similarly as $$h(z)=\sum_{n=0}^{N-1}h_n z^{-n}$$. I dont get it - whats the difference between Z(s) and h(z)?

I think your starting here something big and important! till here it was a general introduction and your starting now a specific graph introduction. Dont you think it deserves a title of its own?
### The Shift operator in Graph Processing
...
> In conclusion:
+ The $$z$$-transform is a key representation for time and frequency analysis in classical DSP.
+ It relies on the definition of the _shift_, or time-delay, operation.
+ The analogy for the shift operation in GSP is a product of the graph signal with a matrix that holds its structural information, such as the adjacency matrix or the Laplacian.
...
when $$\ell$$ is analogous to the frequency $$\lambda_{\ell}$$.

> In conclusion:
+ Eigenvalue decomposition of the shift matrix leads to the spectral representation of graph signal.
+ High and low frequencies of graph signals translate to the number of zero-crossings in the graph spectral components. (You havnt talked about it in this chapter. Maybe you should? this is right in general, not just in graphs).
+ The Fourier basis is the eigenbasis of the time-domain graph's Laplacian.
...
when $$\hat s_{in}=\chi^T s_{in}$$ is the Fourier transform of $$s_{in}$$ (I understand that multipling by $$chi^T$$ is a fourier transform, but I think you should write it down formaly in the last chapter..) and $$\chi$$ is the inverse GFT. It is easier to identify the classic time-domain equations when looking on non-matrix notations
...
> In conclusion:
+ The convolution theorem in DSP translates to practically matrix product of the signal with the spectral components of the Laplacian.
+ Efficient implementation of graph filtering uses a power series of the Laplacian instead of eigen-decomposition.
...
I will first demonstrate visually how the time-series analysis relates to GSP. <u>Notice that plotting code is removed for brevity, but can be found in this [gist](#full_notebook) </u>. (should there be a link here?)
...
for k, eigvec in enumerate(chi.T):
    ...
    # TODO: bug with k=6 (!?) // do you want to leave it like this?
    if k > 5:
        break
```
<p align="center">
...
(Explain you chose this road graph dataset in order to do geo-spatial manipulations that are easy to visualize and nothing more..)
...
We begin by visualizing the graph with geo-spatial context. Every node is located according to it's coordinate features. Remember, <u>the graphical structure itself is solely the connections of the nodes, so that in its processing we are unaware to the geo-spatial location of the node, but only to it's connections</u>. Here we see the graph visualized with and without the geo-spatial context
```python
...
G = graphs.Minnesota()
G_nx = nx.Graph([tuple(r) for r in np.array(G.get_edge_list()[:2]).T]) //What is this? you dont use it..
G.compute_fourier_basis() # caching fourier basis //And this?
```
...
s = np.zeros(G.N)
s += np.sqrt(np.sum((G.coords - np.array([-93.2, 45]))**2, axis=1)s[s>2] = 3
...
That's all for now, hope you enjoyed taking a zip from the beautiful field of GSP. Remember, this blog post is just a very basic introduction to the field. With the resources bellow, you may further dive in for wavelet filter banks, autoregressive modeling of time-graph signals or GSP applications in Big Data constraints.
